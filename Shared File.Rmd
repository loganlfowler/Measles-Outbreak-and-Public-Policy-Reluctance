...
ggplot() +
  geom_sf(data = counties_cov, aes(fill = coverage), color = NA) +
  geom_sf(data = case_points, aes(size = cases),
          shape = 21, fill = "white", color = "black", alpha = 0.85) +
  scale_fill_viridis(option = "C", direction = -1, na.value = "grey92",
                     name = "MMR coverage (%)") +
  scale_size_area(max_size = 7, name = "Measles cases") +
  coord_sf() +
  labs(title = paste("US Measles Cases and Vaccination Coverage,", year_to_map),
       subtitle = "Bubble size = number of cases",
       caption = "Cases: measles_county_all_updates.csv") +
